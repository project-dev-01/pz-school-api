                                ...
                                SUM(
                                    CASE
                                        WHEN lev.to_leave >= lev.from_leave THEN
                                            DATEDIFF(lev.to_leave, lev.from_leave) + 1 -
                                            (
                                                (DATEDIFF(lev.to_leave, lev.from_leave) + 1) / 7 * 2 +
                                                CASE 
                                                    WHEN WEEKDAY(lev.from_leave) <= WEEKDAY(lev.to_leave) THEN
                                                        IF(WEEKDAY(lev.to_leave) = 6, 1, 0) + IF(WEEKDAY(lev.from_leave) = 5, 1, 0)
                                                    ELSE
                                                        IF(WEEKDAY(lev.from_leave) IN (5, 6), 1, 0) + IF(WEEKDAY(lev.to_leave) IN (5, 6), 1, 0)
                                                END
                                            )
                                        ELSE 0
                                    END
                                    ...
                                    WHERE e.holiday = 1 AND e.all_day = "on" AND
                                        (
                                            (e.start_date <= lev.to_leave AND e.end_date >= lev.from_leave) OR
        ...
        try {
            ...
            if (!$validator->passes()) {
                ...
                if (!empty($unhide_data)) {
                    ...
                    // Delete all records if unhide_data is empty
                    $Connection->table('widget_hide_unhide')
                        ->where('staff_id', $staff_id)
                        ->delete();
                }
